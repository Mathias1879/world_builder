...
3. **Uniform stage contract.** Every engine stage: `(grid, relevant constraints, params, LOD) → (layers, verdicts, events)`. Stages are independently testable and replaceable.
...
1 World Model → 2 Edit Log → 3 Constraints → 4 Pipeline (one stage at a time) → 7 Importers (PNG) → 11 Renderer → 10 Editor → 9 Services → 5 Auto-Draw → 8 Exporters → 6 Civilization & Time → 12 AI Chronicle.
...
- **Region rules (fantasy overrides):** painted areas with a rule, e.g. "permanently frozen", "floating isles", "no rain".
...
- Insolation from star luminosity, axial tilt, and seasons; lapse rate; continentality.
...
Peoples/cultures (subsistence, tech era, optional fantasy traits); settlements (point, name, size class, role, optional founding year); nations (name, capital, borders drawn or suggested); routes and landmarks.
...
### 7.6 Time
The calendar derives from planet parameters (year length from orbit, seasons from tilt); users name their eras. A timeline slider scrubs past eras and projected futures. Terrain is fixed on human timescales in v1.
...
- **Planet panel:** radius and gravity; axial tilt and seasons; rotation rate and direction; sea level and ocean coverage; star luminosity and atmosphere/greenhouse; fantasy region rules. Earth defaults.
...
**Importers**
- **v1: PNG/JPG** — automatic land/ocean colour detection with user confirmation; 2:1 aspect assumed whole-planet equirectangular, otherwise the user places the region's lat/lon extent.
