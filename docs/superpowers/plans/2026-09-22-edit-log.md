...
- Produces:
  ...
  - `EditLog::apply_ops(&mut self, Vec<Op>) -> Result<(), EditError>` — atomic: rejects `DuplicateOp`, `UnknownParent`, and malformed ops (`CorruptFile{section:"OPS"}`: parent lamport ≥ child lamport, empty or unsorted/duplicate writes, non-canonical values, label > 200 bytes); on success inserts ops, raises `max_lamport`, and re-materializes the current branch (branch heads are unchanged).
